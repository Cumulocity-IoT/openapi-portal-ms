...
export class C8yBootstrapService {
  ...
  async resolveBootstrapCredentials(): Promise<IMicroserviceSubscription> {
    ...
    const appName = process.env.C8Y_MICROSERVICE_NAME || "swagger-sample-ms";
    ...
    this.logger.log(`Found microservice application '${appName}' with id ${app.id}`);
    const { data } = await client.application.getBootstrapUser(app.id);

    if (!data?.tenant || !data?.name || !data?.password) {
      throw new Error(`Bootstrap credentials for application '${appName}' are incomplete.`);
    }
