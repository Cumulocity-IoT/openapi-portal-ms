...
export class PermissionService {
  ...
  private async hasCurrentUserPermissionAssigned(
    creds: ICredentials,
  ): Promise<boolean> {
    try {
      const client = await this.clientProvider.getUserClient(
        creds.user,
        creds.password,
      );
      ...
      return !!user.effectiveRoles?.find(
        (effectiveRole) => effectiveRole.id === REQUIRED_PERMISSION,
      );
    } catch (e) {
