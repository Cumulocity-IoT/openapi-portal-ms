...
describe('EventsController (empty cache)', () => {
  ...
  it('getEventsV2 applies filter parameter', () => {
    const events = [
      { name: 'click', data: { widgetName: 'w1' }, date: '2020-01-01T00:00:00Z' },
      { name: 'view', data: { widgetName: 'w2' }, date: '2020-01-01T01:00:00Z' },
    ];
    mockCache.queryCache.mockReturnValue(events);
    const res = controller.getEventsV2('2020-01-01', '2020-01-02', 't1234', 'name == "click"', undefined);
    expect(Array.isArray(res)).toBe(true);
  ...
  it('getEventsV2 applies fields parameter', () => {
    ...
    mockCache.queryCache.mockReturnValue(events);
    const res = controller.getEventsV2('2020-01-01', '2020-01-02', 't1234', undefined, 'a');
    expect(Array.isArray(res)).toBe(true);
    expect(res.length).toBe(1);
  ...
  it('getEventsV2 applies filter and fields together', () => {
    ...
    mockCache.queryCache.mockReturnValue(events);
    const res = controller.getEventsV2('2020-01-01', '2020-01-03', 't1234', 'name == "click"', 'a');
    expect(Array.isArray(res)).toBe(true);
