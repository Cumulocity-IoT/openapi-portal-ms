...
describe("ActiveUserControllerV2", () => {
  ...
  describe("getUsersV2 — with data", () => {
    ...
    it("projects only requested fields, always includes id", () => {
      const [result] = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", undefined, "country,browser");
      expect(result.id).toBe("user-1");
      expect(result.country).toBe("Germany");
      expect(result.browser).toBe("Chrome");
    ...
    it("filters by country using filtrex expression", () => {
      mockCache.queryCache.mockReturnValue([makeUser({ iId: "u1", location: { countryName: "Germany" } as any }), makeUser({ iId: "u2", location: { countryName: "France" } as any })]);
      const result = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", 'country == "Germany"');
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe("u1");
    });

    it("returns empty array when filter matches nothing", () => {
      const result = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", 'country == "Mars"');
      expect(result).toEqual([]);
    });

    describe("orderBy", () => {
      beforeEach(() => {
        mockCache.queryCache.mockReturnValue([makeUser({ iId: "u1", location: { countryName: "Germany" } as any }), makeUser({ iId: "u2", location: { countryName: "Austria" } as any })]);
      });

      it("sorts ascending by country with 'country:asc'", () => {
        const result = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", undefined, undefined, "country:asc");
        expect(result[0].id).toBe("u2"); // Austria < Germany
        expect(result[1].id).toBe("u1");
      });

      it("sorts descending by country with 'country:desc'", () => {
        const result = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", undefined, undefined, "country:desc");
        expect(result[0].id).toBe("u1"); // Germany > Austria
      });

      it("defaults to ascending when direction is omitted", () => {
        const result = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", undefined, undefined, "country");
        expect(result[0].id).toBe("u2");
      });

      it("handles extra colon segments safely ('country:asc:extra')", () => {
        const result = controller.getUsersV2("2024-01-01", "2024-01-02", "t1", undefined, undefined, "country:asc:extra");
        expect(result[0].id).toBe("u2");
      });
  ...
  describe("getUsersV2 — defaults for missing attrs", () => {
    it("falls back to '-' for missing country and domain", () => {
      mockCache.queryCache.mockReturnValue([makeUser({ attrs: undefined as any, location: undefined as any })]);
      const [result] = controller.getUsersV2("2024-01-01", "2024-01-02", "t1");
