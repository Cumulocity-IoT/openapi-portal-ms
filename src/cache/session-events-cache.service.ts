...
import { CachedSessionEvent, mapSessionEventsToCachedSessionEvents } from "../model/cache-model";
...
export class SessionEventsCacheService extends ChronoArrayCache<CachedSessionEvent> {
  ...
  queryCache(start: string, end: string, tenantId: string): CachedSessionEvent[] {
  ...
  private async getSessionEventsWithPagination(filter: SessionEventFilter, sum: SessionEvent[], scrollId?: string): Promise<SessionEvent[]> {
    ...
    if (events.length < 1000) {
      ...
      if (!res.scrollId) {
        this.logger.warn(`Received a full page of 1000 session events but scrollId is missing; pagination cannot continue. Total so far: ${sum.length}.`);
        return sum;
      }
      try {
        return await this.getSessionEventsWithPagination(filter, sum, res.scrollId);
      } catch (error) {
